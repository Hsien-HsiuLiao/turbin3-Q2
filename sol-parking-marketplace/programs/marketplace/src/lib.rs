...
pub mod marketplace {
    ...
    pub fn list(ctx: Context<List>, address: String, rental_rate: u16, sensor_id: String, latitude:f64, longitude:f64) -> Result<()> {
        ctx.accounts.create_listing(address, rental_rate, sensor_id, latitude, longitude, &ctx.bumps)?;
        Ok(())
    }
